...
async def send_telegram_alert(message: str, chat_id: str = None) -> bool:
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.post(url, json={"chat_id": target_chat, "text": message, "parse_mode": "Markdown"})
...
async def handle_auth_webhook(
    request: Request, x_webhook_signature: Optional[str] = Header(None, alias="X-Webhook-Signature")
):
    """
...
async def webhook_health():
    ...
    return {"status": "healthy", "endpoint": "/api/v1/webhooks/auth", "signature_required": bool(WEBHOOK_SECRET)}
