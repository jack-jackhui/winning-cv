...
class GenerateSmartCVRequest(BaseModel):
    ...
    base_cv_version_id: Optional[str] = Field(None, description="CV version ID to use as template structure")
...
async def get_kb_stats(user: UserInfo = Depends(get_current_user)) -> KBStatsResponse:
    """
    ...
    return KBStatsResponse(indexed_count=indexed_count, total_sections=total_sections, total_bullets=total_bullets)


@router.post("/index", response_model=IndexCVResponse)
async def index_cv_version(request: IndexCVRequest, user: UserInfo = Depends(get_current_user)) -> IndexCVResponse:
    """
    ...
    try:
        ...
        if not cv_content or not cv_content.strip():
            raise HTTPException(status_code=400, detail="Could not extract text content from CV")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to index CV: {str(e)}")


@router.get("/versions", response_model=list[IndexedVersionResponse])
async def list_indexed_versions(user: UserInfo = Depends(get_current_user)) -> list[IndexedVersionResponse]:
    """
...
async def delete_cv_index(cv_version_id: str, user: UserInfo = Depends(get_current_user)) -> dict:
    """
...
async def search_content(request: SearchRequest, user: UserInfo = Depends(get_current_user)) -> SearchResponse:
    """
    ...
    return SearchResponse(results=[SearchResultItem(**r) for r in results], total=len(results))
...
async def generate_smart_cv(
    request: GenerateSmartCVRequest, user: UserInfo = Depends(get_current_user)
    ...
    try:
        ...
        if not versions and not base_cv_content:
            raise HTTPException(
                status_code=400, detail="No indexed CV versions found. Please index at least one CV first."
            )
        ...
        return GenerateSmartCVResponse(
            cv_content=cv_content, sources_used=len(versions), generation_method="knowledge_base"
        )
...
async def index_all_versions(user: UserInfo = Depends(get_current_user)) -> dict:
    """
    ...
    for version in versions:
        try:
            ...
            failed += 1
            errors.append({"version_id": version.get("version_id") or version.get("id"), "error": str(e)})
