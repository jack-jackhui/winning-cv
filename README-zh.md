...
### 统一职位匹配中心
- **一站式查看** — 所有匹配职位及简历生成状态一目了然
- **智能排序** — 支持按发布时间或匹配分数排序
- **快速统计** — 匹配总数、已生成简历、待生成简历、平均匹配分
- **一键生成简历** — 从任意匹配职位直接生成定制简历
- **状态追踪** — 清晰展示每个职位的简历生成状态和时间
...
5. **浏览匹配** — 查看所有匹配职位，支持按时间或分数排序，清晰展示简历状态
6. **生成定制简历** — 点击"生成简历"为任意职位创建优化简历
7. **下载应用** — 查看、下载或分享您的定制简历
8. **追踪效果** — 通过分析数据了解哪些版本简历效果最佳
...
| `/api/v1/jobs/results` | GET | 获取匹配职位列表（支持`sort_by=date\|score`排序） |
