    ...
    async def index_cv(
        ...
        async with pool.acquire() as conn:
            ...
            async with conn.transaction():
                ...
                if existing:
                    await conn.execute("DELETE FROM cv_extracted_content WHERE id = $1", existing["id"])
        ...
        logger.info(f"Indexed CV {cv_version_id}: {section_count} sections, {bullet_count} bullets")
